...
| 参数 | 默认 | 说明 |
|---|---|---|
| `--log-level=info\|debug` | info | 服务端日志等级。`debug` 时把每一条对局日志（出牌/结算/交互/断线托管等）实时回显到房主控制台，并**完整**追加写入 `devlog/server-log.md`（跨局保留、不截断，已 gitignore） |
...
为防止同一人开多个客户端（CLI/WebUI/Go）双开整人，服务器默认**同一台机器同一时间只允许一个活跃玩家**：来源指纹 = `sha1(IP:机器标识)`，机器标识由客户端持久化（WebUI 存浏览器 localStorage，CLI 存 `~/.clisanguo/machine-id`，中继会把浏览器真实 IP 补上再转发）。因此：

- **同一公网 IP（NAT 后多人）不会互相误伤**——不同机器的机器标识不同、指纹不同，都可正常加入；
- 同一台机器开第二个窗口/终端/标签页会被拒绝（提示“本机已有玩家在线”），关闭一个即可；
- 同一名字在**不同机器**上加入会接管原座位（换设备登录），原连接收到“已在其他设备登录”提示并停止自动重连，不再互相抢座；
- 未发送机器标识的第三方/旧版客户端只按名字规则处理，不受机器校验限制。

CLI 与**轻量 Go 客户端共用同一个 `~/.clisanguo/machine-id` 配置文件**，所以同一台机器上 CLI/Go 任意组合双开都会被识别；WebUI 用浏览器 localStorage 持久化，与 CLI/Go 跨端双开**不同名**时仍可能无法识别（同名始终按一个座位处理）。若房主确实需要同一台机器多开（如本地测试、局域网单机聚会），可用 `--allow-multi-source` 关闭该校验。
...
### Web 浏览器客户端（零依赖 WebUI）

不想装任何东西的话，朋友打开浏览器就能玩。房主额外启动一个 WebUI 中继即可：

```bash
# 终端 1：照常开房
npm run host -- --players=4

# 终端 2：启动 WebUI（默认 http://0.0.0.0:8080，转发到 127.0.0.1:9527）
npm run webui
# 或指定游戏服务器地址与端口：npm run webui -- --host=192.168.1.20 --port=9527 --web-port=8080
```

朋友们在浏览器打开 `http://房主IP:8080`，输入名字即可加入。WebUI 是**纯静态页面 + WebSocket 中继**：`src/webui/relay.ts` 把浏览器的 WebSocket 与游戏服务器的 TCP 协议双向互转（`websocket-server.ts` 为手写的最小 RFC 6455 服务端，零 npm 依赖），页面位于 `webui/`。断线后刷新页面即自动重连，取回 AI 托管的控制权。
...
- 子代理机制：每个 AI 玩家对应一个子代理，包含其身份、武将、角色名，以及**分层策略记忆**（`strategy-memory.ts`：战术笔记 L1 + 跨回合战略方针 L2 + 经验教训 L3 + 执行回看；回合末结构化复盘产出，注入后续决策上下文）。
...
- 动态部分：
  - 最近多轮上下文（默认 30 轮，`SG_AI_CONTEXT_ROUNDS` / `--ai-context-rounds` 可调，适配长上下文模型）：
    - 每轮显示区的出牌/结算内容。
    - 每轮对应的战场状态快照。
  - 策略记忆块（若有）：AI 回合末结构化复盘产出的分层策略笔记（方针→执行→教训→战术），总长有界（≤1000 字符）。
  - 本轮快照：当前行动者、阶段、玩家状态等。
