...
- `src/webui/` + `webui/` — 零依赖 WebUI：`relay.ts` 把浏览器 WebSocket 与游戏服务器 TCP 协议双向互转（每 WS 连接对应一条 TCP 连接，消息原样透传），`websocket-server.ts` 是手写最小 RFC 6455 服务端，静态页面在 `webui/`（vanilla JS，复刻 `client.ts` 的交互流）。`npm run webui` 启动；协议变更时浏览器端只需改 `webui/app.js` 的 `PROTOCOL_VERSION`。
